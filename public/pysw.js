...
addEventListener('message', (event) => {
  ...
  if (data.cmd === 'ps-reset' || data.cmd === 'ps-prerun') {
    ...
    turtlePromiseResolve = null
    if (local) {
      local(new Response(JSON.stringify(data), { status: data?.status ?? 200 }))
    } else {
...
addEventListener('fetch', e => {
  ...
  } else if (e.request.url.includes('bk=') || e.request.url.includes('book=') || (e.request.url.includes('coop=1')) || e.request.url.includes('pyworker') || u.pathname?.startsWith("/static")) {
    e.respondWith(
      fetch(e.request)
